/*This is the program for writing datas into EEPROM via I2C by Bit Banging method.And here time delay is used in milliseconds.
 *
...
uint8_t I2C_WRITE(unsigned char SL_W_ADDR, unsigned char SL_PLACE)
{
	...
	I2C_START();						//Sending the Start Condition.
        ...
        ack = data_write("ATMEGA32");				//Writing the datas.
        if(ack)
        {
                STOP();
...
/*******I2C Portion*************/
	...
	check = I2C_WRITE(0xA0, 0x00);			//Write Function starts.
	if(check == 0)					//If any error occurs program will get terminated.
	{
		return 0;
	}

	check = I2C_READ(0xA0,0x00,0xA1);		//Read Function starts.
	if(check == 0)					//If any error occurs program will get terminated.
	{
		return 0;
	}
