...
int rep_start()
{
        TWCR |= (1<<TWINT)|(1<<TWSTA)|(1<<TWEN);                              //Setting TWI interrupt as 0.
...
void stop()
{
	...
	TWCR |= (1<<TWINT)|(1<<TWSTO)|(1<<TWEN);				//Setting TWI interrupt as 0.
}
...
int address_pass_with_w(unsigned char data)
{
	...
	TWDR = data;  					//move the value to I2C reg.
	TWCR |= (1<<TWINT)|(1<<TWEN)|(1<<TWEA);			//Enable I2C and clear the interrupt.
	while (!(TWCR & (1<<TWINT)));			//Waiting for the current task to over, if over it will become 1 and will come out of 													   the loop,Then the TWDR will become free.
	PORTC = (TWSR & 0xF8);
	_delay_ms(1000);
...
int main(void)
{
	...
	check = address_pass_with_w(0xA0);		//Here we are sending the address of slave(DS1307_RTC-1101000) with write bit(0).
	if(check == 0)
	{
       		send_cmd(CLR_DPLAY_SCR);
		send_cmd(CURSOR_FIRST_LINE);
	...
	check = word_write(0x00); 		//To write data into control register,selecting that address here.
	if(check == 0)
	{
		send_cmd(CLR_DPLAY_SCR);
                send_cmd(CURSOR_FIRST_LINE);
...
/*********************************************************************************************************************/
	...
	check = address_pass_with_w(0xA0);              //Slave address sending with write bit.
        if(check == 0)
        {
                send_cmd(CLR_DPLAY_SCR);
                send_cmd(CURSOR_FIRST_LINE);
	...
	check = address_pass_with_r(0xA1);              //Slave address sending with read bit.
        if(check == 0)
        {
                send_cmd(CLR_DPLAY_SCR);
                send_cmd(CURSOR_FIRST_LINE);
